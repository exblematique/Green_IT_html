...
var CrosshairRenderer = /** @class */ (function () {
    ...
    CrosshairRenderer.prototype.draw = function (ctx) {
        ...
        var vertFix = this._private__data.vertLine.lineWidth % 2 === 0 ? 0.5 : 0;
        var horzFix = this._private__data.horzLine.lineWidth % 2 === 0 ? 0.5 : 0;
        var x = this._private__data.x + 1 + horzFix;
        var y = this._private__data.y + vertFix;
        var w = this._private__data.w;
        var h = this._private__data.h;
        if (vertLinesVisible && x >= 0) {
            ctx.lineWidth = this._private__data.vertLine.lineWidth;
            ctx.strokeStyle = this._private__data.vertLine.color;
            ctx.fillStyle = this._private__data.vertLine.color;
            drawLine(ctx, x, 0, x, h, this._private__data.vertLine.lineStyle);
        }
        if (horzLinesVisible && y >= 0) {
            ctx.lineWidth = this._private__data.horzLine.lineWidth;
            ctx.strokeStyle = this._private__data.horzLine.color;
            ctx.fillStyle = this._private__data.horzLine.color;
            drawLine(ctx, 0, y, w, y, this._private__data.horzLine.lineStyle);
        }
    };
...
var PriceAxisViewRenderer = /** @class */ (function () {
    ...
    PriceAxisViewRenderer.prototype.draw = function (ctx, rendererOptions, textWidthCache, width, align) {
        ...
        var horzBorder = this._private__data.borderVisible ? rendererOptions.borderSize : 0;
        ...
        if (text) {
            ...
            if (this._private__data.tickVisible) {
                ctx.beginPath();
                ctx.strokeStyle = this._private__commonData.color;
...
var PriceAxisView = /** @class */ (function () {
    ...
    PriceAxisView.prototype.floatCoordinate = function () {
        this._private__updateRendererDataIfNeeded();
        return this._private__commonRendererData.floatCoordinate || this._private__commonRendererData.coordinate;
    };
...
var TimeAxisViewRenderer = /** @class */ (function () {
    ...
    TimeAxisViewRenderer.prototype.draw = function (ctx, rendererOptions) {
        ...
        ctx.fill();
        var tickX = Math.round(this._private__data.coordinate + 1);
        var tickTop = y1;
        ...
        ctx.fillText(this._private__data.text, x1 + horzMargin, yText);
    };
...
var Delegate = /** @class */ (function () {
    ...
    Delegate.prototype.fire = function (param1, param2) {
        var listenersSnapshot = this._private__listeners.slice();
    ...
    PriceTickMarkBuilder.prototype.rebuildTickMarks = function () {
        ...
        for (var logical = high - mod; logical > low; logical -= span) {
            ...
            if (targetIndex < this._private__marks.length) {
                this._private__marks[targetIndex].coord = Math.round(coord);
...
var PaneRendererBars = /** @class */ (function () {
    ...
    PaneRendererBars.prototype.setData = function (data) {
        this._private__data = data;
        this._private__barWidth = optimalBarWidth(data.barSpacing);
        this._private__barLineWidth = data.thinBars ? 1 : Math.max(1, Math.round(this._private__barWidth));
    };
    PaneRendererBars.prototype.draw = function (ctx) {
        ...
        var offset = this._private__data.thinBars ? 1 : Math.round(this._private__barWidth);
        var negativeOffset = this._private__data.thinBars ? 1 : offset / 2;
        var prevColor = null;
        for (var i = this._private__data.visibleRange.from; i < this._private__data.visibleRange.to; ++i) {
            ...
            ctx.fillRect(Math.round(bar.x - this._private__barLineWidth / 2), Math.round(bar.highY - negativeOffset), Math.round(this._private__barLineWidth), Math.round(bar.lowY - bar.highY + offset));
            if (this._private__barLineWidth < (this._private__data.barSpacing - 1)) {
                if (this._private__data.openVisible) {
                    ctx.fillRect(Math.round(bar.x - this._private__barWidth * 1.5), Math.floor(bar.openY - negativeOffset), Math.round(this._private__barWidth * 1.5), offset);
                }
                ctx.fillRect(Math.round(bar.x), Math.floor(bar.closeY - negativeOffset), Math.round(this._private__barWidth * 1.5), offset);
            }
        }
        ctx.restore();
    };
...
var PaneRendererCandlesticks = /** @class */ (function () {
    ...
    PaneRendererCandlesticks.prototype.draw = function (ctx) {
        ...
        ctx.lineCap = 'square';
        if (this._private__data.barSpacing < 1) {
            this._private__drawThinCandles(ctx);
        }
        else {
    ...
    PaneRendererCandlesticks.prototype._private__drawBorder = function (ctx, bars, visibleRange) {
        var prevBorderColor = '';
        for (var i = visibleRange.from; i < visibleRange.to; i++) {
            var bar = bars[i];
            ...
            var left = Math.round(bar.x - this._private__barWidth);
            var right = Math.round(bar.x + this._private__barWidth);
    ...
    PaneRendererCandlesticks.prototype._private__drawCandles = function (ctx, bars, visibleRange) {
        if (this._private__data === null) {
            return;
        }
        var prevBarColor = '';
    ...
    PaneRendererCandlesticks.prototype._private__drawThinCandles = function (ctx) {
        ...
        if (this._private__data.wickVisible) {
            ctx.strokeStyle = this._private__data.wickColor;
            ctx.beginPath();
            for (var i = this._private__data.visibleRange.from; i < this._private__data.visibleRange.to; i++) {
                var bar = this._private__data.bars[i];
...
var PaneRendererHistogram = /** @class */ (function () {
    ...
    PaneRendererHistogram.prototype.draw = function (ctx) {
        ...
        var histogramBase = this._private__data.histogramBase;
        ctx.translate(0.5, 0.5);
        ctx.fillStyle = this._private__data.color;
        ctx.lineWidth = 1;
        ctx.beginPath();
...
var SeriesHistogramPaneView = /** @class */ (function (_super) {
    ...
    function SeriesHistogramPaneView(series, model) {
        ...
        _this._private__paletteData = [];
        _this._private__paletteRenderers = [];
        _this._private__colorIndexes = new Int32Array(0);
        _this._private__sourceIndexes = new Int32Array(0);
        return _this;
    }
    ...
    SeriesHistogramPaneView.prototype._fillRawPoints = function () {
        ...
        // resize arrays
        this._private__paletteRenderers.length = palette.size();
        this._private__paletteData.length = palette.size();
        var targetIndexes = new Int32Array(palette.size() + 1);
        ...
        this._series.bars().each(function (index, bar) {
            ...
            var data = _this._private__paletteData[colorIndex] || createEmptyHistogramData(barSpacing, histogramStyleProps.lineWidth, color);
            ...
            _this._private__paletteData[colorIndex] = data;
            _this._private__colorIndexes[targetColorIndex++] = colorIndex;
            return false;
        });
        // update renderers
        this._private__paletteRenderers.length = this._private__paletteData.length;
        if (this._private__sourceIndexes.length !== this._private__paletteData.length) {
            this._private__sourceIndexes = new Int32Array(this._private__paletteData.length);
        }
        this._private__paletteData.forEach(function (element, index) {
            element.items.length = targetIndexes[index];
            var renderer = _this._private__paletteRenderers[index] || new PaneRendererHistogram();
            renderer.setData(element);
            _this._private__paletteRenderers[index] = renderer;
        });
        this._private__compositeRenderer.setRenderers(this._private__paletteRenderers);
    };
    SeriesHistogramPaneView.prototype._clearVisibleRange = function () {
        _super.prototype._clearVisibleRange.call(this);
        this._private__paletteData.forEach(function (data, colorIndex) {
            data.visibleRange = null;
        });
    };
    SeriesHistogramPaneView.prototype._convertToCoordinates = function (priceScale, timeScale, firstValue) {
        ...
        this._private__paletteData.forEach(function (data, colorIndex) {
            ...
            _this._private__sourceIndexes[colorIndex] = data.visibleRange.from;
        });
        // now calculate left and right
        var prevItem = null;
        for (var i = this._itemsVisibleRange.from; i < this._itemsVisibleRange.to; ++i) {
            var colorIndex = this._private__colorIndexes[i];
            var data = this._private__paletteData[colorIndex];
            var sourceIndex = this._private__sourceIndexes[colorIndex]++;
...
var PanePriceAxisViewRenderer = /** @class */ (function () {
    ...
    PanePriceAxisViewRenderer.prototype.draw = function (ctx) {
        ...
        this._private__priceAxisViewRenderer.draw(ctx, this._private__rendererOptions, this._private__textWidthCache, this._private__width, this._private__align);
    };
...
var HorizontalLineRenderer = /** @class */ (function () {
    ...
    HorizontalLineRenderer.prototype.draw = function (ctx) {
        ...
        var y = this._private__data.y;
        if (y < 0 || y > this._private__data.height) {
            return;
        }
        var width = this._private__data.width;
        ctx.lineCap = 'square';
        ctx.strokeStyle = this._private__data.color;
        ctx.lineWidth = this._private__data.lineWidth;
        drawLine(ctx, 0, y, width, y, this._private__data.lineStyle);
    };
...
function ceilToEven(x) {
    var ceiled = Math.ceil(x);
    return (ceiled % 2 !== 0) ? ceiled - 1 : ceiled;
}
function ceilToOdd(x) {
    var ceiled = Math.ceil(x);
    return (ceiled % 2 === 0) ? ceiled - 1 : ceiled;
}
...
var SeriesMarkersRenderer = /** @class */ (function () {
    ...
    SeriesMarkersRenderer.prototype.draw = function (ctx, isHovered, hitTestData) {
        if (this._private__data === null || this._private__data.visibleRange === null) {
            return;
        }
        ctx.save();
...
var SeriesMarkersPaneView = /** @class */ (function () {
    ...
    SeriesMarkersPaneView.prototype._makeValid = function () {
        ...
        if (seriesMarkers.length > 0) {
            var marginsAboveAndBelow = calculateShapeHeight(timeScale.barSpacing()) * 1.5 + shapeMargin$1 * 2;
...
var Series = /** @class */ (function (_super) {
    ...
    function Series(model, options, seriesType) {
        ...
        _this._private__markersPaneView = new SeriesMarkersPaneView(_this, model);
        return _this;
    }
    ...
    Series.prototype.setData = function (data) {
        ...
        if (this._private__paneView !== null) {
    ...
    Series.prototype.paneViews = function () {
        ...
        res.push(ensureNotNull(this._private__paneView));
    ...
    Series.prototype.priceAxisViews = function (pane, priceScale) {
        return this._private__priceAxisViews;
    };
    ...
    Series.prototype.updateAllViews = function () {
        ...
        this._private__markersPaneView.update();
        var priceAxisViewsLength = this._private__priceAxisViews.length;
        for (var i = 0; i < priceAxisViewsLength; i++) {
            this._private__priceAxisViews[i].update();
        }
...
var PriceScale = /** @class */ (function () {
    ...
    PriceScale.prototype.priceToCoordinate = function (price, baseValue, keepItFloat) {
        ...
        return this._private__logicalToCoordinate(price, baseValue, keepItFloat);
    };
    ...
    PriceScale.prototype._private__logicalToCoordinate = function (logical, baseValue, keepItFloat) {
        this._private__makeSureItIsValid();
        if (this.isEmpty()) {
            return 0;
        }
...
var CanvasWrapper = /** @class */ (function () {
    function CanvasWrapper(originalContext, ratio) {
        ...
        this._private__ctx = originalContext;
        this._private__rto = ratio;
    }
    CanvasWrapper.prototype.restore = function () {
        this._private__ctx.restore();
    };
    CanvasWrapper.prototype.save = function () {
        this._private__ctx.save();
    };
    CanvasWrapper.prototype.getTransform = function () {
        return this._private__ctx.getTransform();
    };
    CanvasWrapper.prototype.resetTransform = function () {
        this._private__ctx.resetTransform();
    };
    CanvasWrapper.prototype.rotate = function (angle) {
        this._private__ctx.rotate(angle);
    };
    CanvasWrapper.prototype.scale = function (x, y) {
        this._private__ctx.scale(x, y);
    };
    CanvasWrapper.prototype.setTransform = function (a, b, c, d, e, f) {
        if (isNumber(a)) {
            this._private__ctx.setTransform(a, b, c, d, e, f);
        }
        else {
            this._private__ctx.setTransform(a);
        }
    };
    CanvasWrapper.prototype.transform = function (a, b, c, d, e, f) {
        this._private__ctx.transform(a, b, c, d, e, f);
    };
    CanvasWrapper.prototype.translate = function (x, y) {
        this._private__ctx.translate(x * this._private__rto, y * this._private__rto);
    };
    Object.defineProperty(CanvasWrapper.prototype, "globalAlpha", {
        get: function () {
            return this._private__ctx.globalAlpha;
        },
        set: function (value) {
            this._private__ctx.globalAlpha = value;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(CanvasWrapper.prototype, "globalCompositeOperation", {
        get: function () {
            return this._private__ctx.globalCompositeOperation;
        },
        set: function (value) {
            this._private__ctx.globalCompositeOperation = value;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(CanvasWrapper.prototype, "imageSmoothingEnabled", {
        get: function () {
            return this._private__ctx.imageSmoothingEnabled;
        },
        set: function (value) {
            this._private__ctx.imageSmoothingEnabled = value;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(CanvasWrapper.prototype, "imageSmoothingQuality", {
        get: function () {
            return this._private__ctx.imageSmoothingQuality;
        },
        set: function (value) {
            this._private__ctx.imageSmoothingQuality = value;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(CanvasWrapper.prototype, "fillStyle", {
        get: function () {
            return this._private__ctx.fillStyle;
        },
        set: function (value) {
            this._private__ctx.fillStyle = value;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(CanvasWrapper.prototype, "strokeStyle", {
        get: function () {
            return this._private__ctx.strokeStyle;
        },
        set: function (value) {
            this._private__ctx.strokeStyle = value;
        },
        enumerable: true,
        configurable: true
    });
    CanvasWrapper.prototype.createLinearGradient = function (x0, y0, x1, y1) {
        return this._private__ctx.createLinearGradient(x0 * this._private__rto, y0 * this._private__rto, x1 * this._private__rto, y1 * this._private__rto);
    };
    CanvasWrapper.prototype.createPattern = function (image, repetition) {
        return this._private__ctx.createPattern(image, repetition);
    };
    CanvasWrapper.prototype.createRadialGradient = function (x0, y0, r0, x1, y1, r1) {
        return this._private__ctx.createRadialGradient(x0 * this._private__rto, y0 * this._private__rto, r0 * this._private__rto, x1 * this._private__rto, y1 * this._private__rto, r1 * this._private__rto);
    };
    Object.defineProperty(CanvasWrapper.prototype, "shadowBlur", {
        get: function () {
            return this._private__ctx.shadowBlur;
        },
        set: function (value) {
            this._private__ctx.shadowBlur = value;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(CanvasWrapper.prototype, "shadowColor", {
        get: function () {
            return this._private__ctx.shadowColor;
        },
        set: function (value) {
            this._private__ctx.shadowColor = value;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(CanvasWrapper.prototype, "shadowOffsetX", {
        get: function () {
            return this._private__ctx.shadowOffsetX;
        },
        set: function (value) {
            this._private__ctx.shadowOffsetX = value;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(CanvasWrapper.prototype, "shadowOffsetY", {
        get: function () {
            return this._private__ctx.shadowOffsetY;
        },
        set: function (value) {
            this._private__ctx.shadowOffsetY = value;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(CanvasWrapper.prototype, "filter", {
        get: function () {
            return this._private__ctx.filter;
        },
        set: function (value) {
            this._private__ctx.filter = value;
        },
        enumerable: true,
        configurable: true
    });
    CanvasWrapper.prototype.clearRect = function (x, y, w, h) {
        this._private__ctx.clearRect(x * this._private__rto, y * this._private__rto, w * this._private__rto, h * this._private__rto);
    };
    CanvasWrapper.prototype.fillRect = function (x, y, w, h) {
        this._private__ctx.fillRect(x * this._private__rto, y * this._private__rto, w * this._private__rto, h * this._private__rto);
    };
    CanvasWrapper.prototype.strokeRect = function (x, y, w, h) {
        this._private__ctx.strokeRect(x * this._private__rto, y * this._private__rto, w * this._private__rto, h * this._private__rto);
    };
    CanvasWrapper.prototype.beginPath = function () {
        this._private__ctx.beginPath();
    };
    CanvasWrapper.prototype.clip = function (path, fillRule) {
        if (path === 'nonzero' || path === 'evenodd' || path === undefined) {
            this._private__ctx.clip(path);
        }
        else {
            this._private__ctx.clip(path, fillRule);
        }
    };
    CanvasWrapper.prototype.fill = function (path, fillRule) {
        if (path === 'nonzero' || path === 'evenodd' || path === undefined) {
            this._private__ctx.fill(path);
        }
        else {
            this._private__ctx.fill(path, fillRule);
        }
    };
    CanvasWrapper.prototype.isPointInPath = function (path, x, y, fillRule) {
        return isNumber(path) ?
            this._private__ctx.isPointInPath(path * this._private__rto, x * this._private__rto, y) :
            this._private__ctx.isPointInPath(path, x * this._private__rto, y * this._private__rto, fillRule);
    };
    CanvasWrapper.prototype.isPointInStroke = function (path, x, y) {
        return isNumber(path) ?
            this._private__ctx.isPointInStroke(path * this._private__rto, x * this._private__rto) :
            this._private__ctx.isPointInStroke(path, x * this._private__rto, y * this._private__rto);
    };
    CanvasWrapper.prototype.stroke = function (path) {
        if (path === undefined) {
            this._private__ctx.stroke();
            return;
        }
        this._private__ctx.stroke(path);
    };
    CanvasWrapper.prototype.drawFocusIfNeeded = function (path, element) {
        if (element !== undefined) {
            this._private__ctx.drawFocusIfNeeded(path, element);
        }
        else {
            this._private__ctx.drawFocusIfNeeded(path);
        }
    };
    CanvasWrapper.prototype.scrollPathIntoView = function (path) {
        if (path !== undefined) {
            this._private__ctx.scrollPathIntoView(path);
        }
        else {
            this._private__ctx.scrollPathIntoView();
        }
    };
    CanvasWrapper.prototype.fillText = function (text, x, y, maxWidth) {
        var _this = this;
        if (this._private__rto !== 1) {
            this.font = this.font.replace(fontSizeRegex, function (w, m, u) {
                return (m * _this._private__rto) + u;
            });
        }
        if (maxWidth === undefined) {
            this._private__ctx.fillText(text, x * this._private__rto, y * this._private__rto);
        }
        else {
            this._private__ctx.fillText(text, x * this._private__rto, y * this._private__rto, maxWidth * this._private__rto);
        }
        if (this._private__rto !== -1) {
            this.font = this.font.replace(fontSizeRegex, function (w, m, u) {
                return (m / _this._private__rto) + u;
    ...
    CanvasWrapper.prototype.measureText = function (text) {
        return this._private__ctx.measureText(text);
    };
    CanvasWrapper.prototype.strokeText = function (text, x, y, maxWidth) {
        ...
        if (maxWidth === undefined) {
            this._private__ctx.strokeText(text, x * this._private__rto, y * this._private__rto);
        }
        else {
            this._private__ctx.strokeText(text, x * this._private__rto, y * this._private__rto, maxWidth * this._private__rto);
        }
        if (this._private__rto !== 1) {
            this.font = this.font.replace(fontSizeRegex, function (w, m, u) {
                return (m / _this._private__rto) + u;
    ...
    CanvasWrapper.prototype.drawImage = function (image, sx, sy, sw, sh, dx, dy, dw, dh) {
        if (image instanceof HTMLCanvasElement) {
            return this._private__drawImagePatchedSourceAndDest(image, sx, sy, sw, sh, dx, dy, dw, dh);
        }
        else {
            return this._private__drawImagePatchedSource(image, sx, sy, sw, sh, dx, dy, dw, dh);
        }
    };
    CanvasWrapper.prototype.createImageData = function (sw, sh) {
        if (isNumber(sw)) {
            return this._private__ctx.createImageData(sw, sh);
        }
        return this._private__ctx.createImageData(sw);
    };
    CanvasWrapper.prototype.getImageData = function (sx, sy, sw, sh) {
        return this._private__ctx.getImageData(sx, sy, sw, sh);
    };
    // tslint:disable-next-line:max-params
    CanvasWrapper.prototype.putImageData = function (imagedata, dx, dy, dirtyX, dirtyY, dirtyWidth, dirtyHeight) {
        if (dirtyX === undefined) {
            this._private__ctx.putImageData(imagedata, dx, dy);
        }
        else {
            this._private__ctx.putImageData(imagedata, dx, dy, dirtyX, dirtyY, dirtyWidth, dirtyHeight);
        }
    };
    Object.defineProperty(CanvasWrapper.prototype, "lineCap", {
        get: function () {
            return this._private__ctx.lineCap;
        },
        set: function (value) {
            this._private__ctx.lineCap = value;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(CanvasWrapper.prototype, "lineDashOffset", {
        get: function () {
            return this._private__ctx.lineDashOffset;
        },
        set: function (value) {
            this._private__ctx.lineDashOffset = value;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(CanvasWrapper.prototype, "lineJoin", {
        get: function () {
            return this._private__ctx.lineJoin;
        },
        set: function (value) {
            this._private__ctx.lineJoin = value;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(CanvasWrapper.prototype, "lineWidth", {
        get: function () {
            return this._private__ctx.lineWidth;
        },
        set: function (value) {
            this._private__ctx.lineWidth = value;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(CanvasWrapper.prototype, "miterLimit", {
        get: function () {
            return this._private__ctx.miterLimit;
        },
        set: function (value) {
            this._private__ctx.miterLimit = value;
        },
        enumerable: true,
        configurable: true
    });
    CanvasWrapper.prototype.getLineDash = function () {
        return this._private__ctx.getLineDash();
    };
    CanvasWrapper.prototype.setLineDash = function (segments) {
        this._private__ctx.setLineDash(segments);
    };
    Object.defineProperty(CanvasWrapper.prototype, "direction", {
        get: function () {
            return this._private__ctx.direction;
        },
        set: function (value) {
            this._private__ctx.direction = value;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(CanvasWrapper.prototype, "font", {
        get: function () {
            return this._private__ctx.font;
        },
        set: function (value) {
            this._private__ctx.font = value;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(CanvasWrapper.prototype, "textAlign", {
        get: function () {
            return this._private__ctx.textAlign;
        },
        set: function (value) {
            this._private__ctx.textAlign = value;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(CanvasWrapper.prototype, "textBaseline", {
        get: function () {
            return this._private__ctx.textBaseline;
        },
        set: function (value) {
            this._private__ctx.textBaseline = value;
        },
        enumerable: true,
        configurable: true
    });
    CanvasWrapper.prototype.arc = function (x, y, radius, startAngle, endAngle, anticlockwise) {
        this._private__ctx.arc(x * this._private__rto, y * this._private__rto, radius * this._private__rto, startAngle, endAngle, anticlockwise);
    };
    CanvasWrapper.prototype.arcTo = function (x1, y1, x2, y2, radius) {
        this._private__ctx.arcTo(x1 * this._private__rto, y1 * this._private__rto, x2 * this._private__rto, y2 * this._private__rto, radius * this._private__rto);
    };
    CanvasWrapper.prototype.bezierCurveTo = function (cp1x, cp1y, cp2x, cp2y, x, y) {
        this._private__ctx.bezierCurveTo(cp1x * this._private__rto, cp1y * this._private__rto, cp2x * this._private__rto, cp2y * this._private__rto, x * this._private__rto, y * this._private__rto);
    };
    CanvasWrapper.prototype.closePath = function () {
        this._private__ctx.closePath();
    };
    // tslint:disable-next-line:max-params
    CanvasWrapper.prototype.ellipse = function (x, y, radiusX, radiusY, rotation, startAngle, endAngle, anticlockwise) {
        this._private__ctx.ellipse(x, y, radiusX, radiusY, rotation, startAngle, endAngle, anticlockwise);
    };
    CanvasWrapper.prototype.lineTo = function (x, y) {
        this._private__ctx.lineTo(x * this._private__rto, y * this._private__rto);
    };
    CanvasWrapper.prototype.moveTo = function (x, y) {
        this._private__ctx.moveTo(x * this._private__rto, y * this._private__rto);
    };
    CanvasWrapper.prototype.quadraticCurveTo = function (cpx, cpy, x, y) {
        this._private__ctx.quadraticCurveTo(cpx * this._private__rto, cpy * this._private__rto, x * this._private__rto, y * this._private__rto);
    };
    CanvasWrapper.prototype.rect = function (x, y, w, h) {
        this._private__ctx.rect(x * this._private__rto, y * this._private__rto, w * this._private__rto, h * this._private__rto);
    };
    CanvasWrapper.prototype._private__drawImageImpl = function (image, sx, sy, sw, sh, dx, dy, dw, dh) {
        if (sw === undefined) {
            this._private__ctx.drawImage(image, sx, sy);
        }
        else if (dx === undefined) {
            this._private__ctx.drawImage(image, sx, sy, sw, sh);
        }
        else {
            this._private__ctx.drawImage(image, sx, sy, sw, sh, dx, dy, dw, dh);
        }
    };
    CanvasWrapper.prototype._private__drawImagePatchedSource = function (image, sx, sy, sw, sh, dx, dy, dw, dh) {
        this._private__drawImageImpl(image, sx * this._private__rto, sy * this._private__rto, sw === undefined ? sw : sw * this._private__rto, sh === undefined ? sh : sh * this._private__rto, dx === undefined ? dx : dx * this._private__rto, dy === undefined ? dy : dy * this._private__rto, dw === undefined ? dw : dw * this._private__rto, dh === undefined ? dh : dh * this._private__rto);
    };
    CanvasWrapper.prototype._private__drawImagePatchedSourceAndDest = function (image, sx, sy, sw, sh, dx, dy, dw, dh) {
...
var GridRenderer = /** @class */ (function () {
    ...
    GridRenderer.prototype.draw = function (ctx) {
        ...
        ctx.lineWidth = 1;
        if (this._private__data.vertLinesVisible) {
            ctx.strokeStyle = this._private__data.vertLinesColor;
            setLineStyle(ctx, this._private__data.vertLineStyle);
            ctx.beginPath();
            for (var _i = 0, _a = this._private__data.timeMarks; _i < _a.length; _i++) {
                var timeMark = _a[_i];
                ctx.moveTo(timeMark.coord + 1, 0);
                ctx.lineTo(timeMark.coord + 1, this._private__data.h);
            }
            ctx.stroke();
        }
        if (this._private__data.horzLinesVisible) {
            ctx.strokeStyle = this._private__data.horzLinesColor;
            setLineStyle(ctx, this._private__data.horzLineStyle);
            ctx.beginPath();
            for (var _b = 0, _c = this._private__data.priceMarks; _b < _c.length; _b++) {
                var priceMark = _c[_b];
                ctx.moveTo(0, priceMark.coord);
                ctx.lineTo(this._private__data.w, priceMark.coord);
            }
            ctx.stroke();
        }
    };
...
var Magnet = /** @class */ (function () {
    ...
    Magnet.prototype.align = function (price, index, pane) {
        ...
        var candidates = serieses.reduce(function (acc, series) {
            ...
            return acc.concat(prices.map(function (barPrice) { return ps.priceToCoordinate(barPrice, firstPrice.value, true); }));
        }, []);
...
var Pane = /** @class */ (function () {
    ...
    Pane.prototype.destroy = function () {
        ...
        this._private__timeScale.barSpacingChanged().unsubscribeAll(this);
    ...
    Pane.prototype.recalculate = function () {
        ...
        this.updateAllViews();
        this._private__model.updatePane(this);
    };
    ...
    Pane.prototype._private__findSuitableScale = function (source, preferredScale) {
        ...
        return this._private__createPriceScale();
    };
    ...
    Pane.prototype._private__createPriceScale = function () {
        var priceScale = new PriceScale(clone(this._private__model.options().priceScale), this._private__model.options().layout, this._private__model.options().localization);
...
var TimeScale = /** @class */ (function () {
    function TimeScale(model, options, localizationOptions) {
        ...
        this._private__barSpacingChanged = new Delegate();
        this._private__scrollStartPoint = null;
        this._private__scaleStartPoint = null;
        ...
        this._private__barSpacing = getValidBarSpacing(options.barSpacing);
        this._private__model = model;
    ...
    TimeScale.prototype.barSpacingChanged = function () {
        return this._private__barSpacingChanged;
    };
    ...
    TimeScale.prototype.fitContent = function () {
        ...
        this.setVisibleRange(new BarsRange(first - 1, last + 1 + this._private__options.rightOffset));
    };
    ...
    TimeScale.prototype._private__setBarSpacing = function (newBarSpacing) {
        ...
        this._private__barSpacingChanged.fire(oldBarSpacing, newBarSpacing);
    ...
    TimeScale.prototype._private__maxRightOffset = function () {
        return (this._private__width / this._private__barSpacing) - 5 /* MinVisibleBarsCount */;
    };
...
var ChartModel = /** @class */ (function () {
    ...
    ChartModel.prototype.updatePane = function (pane) {
        var inv = this._private__paneInvalidationMask(pane, 0 /* None */);
...
var PaneSeparator = /** @class */ (function () {
    ...
    PaneSeparator.prototype.getImage = function () {
        ...
        var width = this._private__paneA.getSize().w;
...
var LabelsImageCache = /** @class */ (function () {
    ...
    LabelsImageCache.prototype.getLabelImage = function (ctx, text) {
        ...
        if (this._private__hash.has(text)) {
            ...
            var margin = Math.ceil(this._private__fontSize / 4.5);
            var baselineOffset = Math.round(this._private__fontSize / 10);
            ...
            item.height = this._private__fontSize + margin * 2;
            if (textWidth !== 0) {
...
var PriceAxisWidget = /** @class */ (function () {
    function PriceAxisWidget(pane, options, rendererOptionsProvider, side) {
        ...
        this._private__canvas = addCanvasTo(this._private__cell, new Size(16, 16));
        this._private__canvas.style.position = 'absolute';
        this._private__canvas.style.zIndex = '1';
        this._private__canvas.style.left = '0';
        this._private__canvas.style.top = '0';
        this._private__ctx = ensureNotNull(getContext2d(this._private__canvas));
        this._private__topCanvas = addCanvasTo(this._private__cell, new Size(16, 16));
        this._private__topCanvas.style.position = 'absolute';
        this._private__topCanvas.style.zIndex = '2';
        this._private__topCanvas.style.left = '0';
        this._private__topCanvas.style.top = '0';
        this._private__topCtx = ensureNotNull(getContext2d(this._private__topCanvas));
        var handler = {
            mouseDownEvent: this._private__mouseDownEvent.bind(this),
            pressedMouseMoveEvent: this._private__pressedMouseMoveEvent.bind(this),
        ...
        this._private__mouseEventHandler = new MouseEventHandler(this._private__topCanvas, handler, {
            treatVertTouchDragAsPageScroll: false,
            treatHorzTouchDragAsPageScroll: true,
        });
    }
    ...
    PriceAxisWidget.prototype.setSize = function (size) {
        ...
        if (this._private__size === null || !this._private__size.equals(size)) {
            ...
            resizeCanvas(this._private__canvas, size);
            resizeCanvas(this._private__topCanvas, size);
    ...
    PriceAxisWidget.prototype.paint = function (type) {
        ...
        this._private__topCtx.clearRect(-0.5, -0.5, this._private__size.w, this._private__size.h);
        if (type !== 1 /* Cursor */) {
            this._private__alignLabels();
            this._private__drawBackground(this._private__ctx);
            this._private__drawBorder(this._private__ctx);
            this._private__drawTickMarks(this._private__ctx);
            this._private__drawBackLabels(this._private__ctx);
        }
        this._private__drawCrosshairLabel(this._private__topCtx);
    };
    PriceAxisWidget.prototype.getImage = function () {
        return this._private__canvas;
    };
    ...
    PriceAxisWidget.prototype._private__drawBackground = function (ctx) {
        if (this._private__size === null) {
            return;
        }
        clearRect(ctx, 0, 0, this._private__size.w, this._private__size.h, this.backgroundColor());
    };
    PriceAxisWidget.prototype._private__drawBorder = function (ctx) {
        ...
        if (this._private__isLeft) {
            ctx.translate(-0.5, -0.5);
            left = this._private__size.w - borderSize - 1;
        }
        else {
            ctx.translate(0.5, -0.5);
            left = 0;
        }
        ctx.fillRect(left, 0, borderSize, this._private__size.h);
        ctx.restore();
    };
    PriceAxisWidget.prototype._private__drawTickMarks = function (ctx) {
        ...
        var tickMarkLeftX = this._private__isLeft ?
            this._private__size.w - rendererOptions.offsetSize - rendererOptions.borderSize - rendererOptions.tickLength :
        ...
        for (var _a = 0, tickMarks_2 = tickMarks; _a < tickMarks_2.length; _a++) {
            ...
            this._private__tickMarksCache.paintTo(ctx, tickMark.label, textLeftX, tickMark.coord, textAlign);
        }
        ctx.restore();
    };
    ...
    PriceAxisWidget.prototype._private__drawBackLabels = function (ctx) {
        var _this = this;
        if (this._private__size === null) {
            return;
        }
        ...
        views.forEach(function (view) {
            if (view.isAxisLabelVisible()) {
                ...
                ctx.save();
                renderer.draw(ctx, rendererOptions, _this._private__widthCache, size.w, align);
                ctx.restore();
            }
        });
    };
    PriceAxisWidget.prototype._private__drawCrosshairLabel = function (ctx) {
        var _this = this;
        if (this._private__size === null || this._private__priceScale === null) {
            return;
        }
        ...
        views.forEach(function (arr) {
            arr.forEach(function (view) {
                ctx.save();
                view.renderer().draw(ctx, ro, _this._private__widthCache, size.w, align);
                ctx.restore();
            });
        });
    };
...
var PaneWidget = /** @class */ (function () {
    function PaneWidget(chart, state) {
        ...
        this._private__canvas = addCanvasTo(paneWrapper, new Size(16, 16));
        this._private__canvas.style.position = 'absolute';
        this._private__canvas.style.zIndex = '1';
        this._private__canvas.style.left = '0';
        this._private__canvas.style.top = '0';
        this._private__ctx = ensureNotNull(getContext2d(this._private__canvas));
        this._private__topCanvas = addCanvasTo(paneWrapper, new Size(16, 16));
        this._private__topCanvas.style.position = 'absolute';
        this._private__topCanvas.style.zIndex = '2';
        this._private__topCanvas.style.left = '0';
        this._private__topCanvas.style.top = '0';
        ...
        this._private__mouseEventHandler = new MouseEventHandler(this._private__topCanvas, this, {
    ...
    PaneWidget.prototype.paint = function (type) {
        ...
        this._private__topCtx.clearRect(-0.5, -0.5, this._private__size.w, this._private__size.h);
        if (type !== 1 /* Cursor */) {
            this._private__drawBackground(this._private__ctx, this._private__backgroundColor());
            if (this._private__state) {
                this._private__drawGrid(this._private__ctx);
                this._private__drawWatermark(this._private__ctx);
                this._private__drawSources(this._private__ctx);
            }
        }
        this._private__drawCrosshair(this._private__topCtx);
    };
    ...
    PaneWidget.prototype._private__drawBackground = function (ctx, color) {
        clearRect(ctx, 0, 0, this._private__size.w, this._private__size.h, color);
    };
    PaneWidget.prototype._private__drawGrid = function (ctx) {
    ...
    PaneWidget.prototype._private__drawWatermark = function (ctx) {
    ...
    PaneWidget.prototype._private__drawCrosshair = function (ctx) {
        this._private__drawSource(this._private__model().crosshairSource(), ctx);
    };
    PaneWidget.prototype._private__drawSources = function (ctx) {
        var state = ensureNotNull(this._private__state);
        var sources = state.orderedSources();
        ...
        for (var _i = 0, sources_2 = sources; _i < sources_2.length; _i++) {
            ...
            this._private__drawSourceBackground(source, ctx);
        }
        for (var _a = 0, sources_3 = sources; _a < sources_3.length; _a++) {
            var source = sources_3[_a];
            if (source !== crosshairSource) {
                this._private__drawSource(source, ctx);
            }
        }
    };
    PaneWidget.prototype._private__drawSource = function (source, ctx) {
        var state = ensureNotNull(this._private__state);
        var paneViews = source.paneViews(state);
        var height = state.height();
        var width = state.width();
    ...
    PaneWidget.prototype._private__drawSourceBackground = function (source, ctx) {
        var state = ensureNotNull(this._private__state);
        var paneViews = source.paneViews(state);
        var height = state.height();
        var width = state.width();
...
var PriceAxisStub = /** @class */ (function () {
    function PriceAxisStub(side, options, params, borderVisible) {
        ...
        this._private__cell.style.overflow = 'hidden';
        this._private__canvas = addCanvasTo(this._private__cell, new Size(16, 16));
        this._private__ctx = getContext2d(this._private__canvas);
    }
    ...
    PriceAxisStub.prototype.paint = function (type) {
        ...
        this._private__invalidated = false;
        this._private__drawBackground();
        this._private__drawBorder();
    };
    ...
    PriceAxisStub.prototype._private__drawBorder = function () {
        if (!this._private__borderVisible()) {
            return;
        }
        var width = this._private__size.w;
        this._private__ctx.save();
        this._private__ctx.fillStyle = this._private__options.timeScale.borderColor;
        var borderSize = this._private__rendererOptionsProvider.options().borderSize;
        var left;
        if (this._private__isLeft) {
            this._private__ctx.translate(-0.5, -0.5);
        ...
        this._private__ctx.fillRect(left, 0, borderSize * 2, 1);
        this._private__ctx.restore();
    };
    PriceAxisStub.prototype._private__drawBackground = function () {
        clearRect(this._private__ctx, 0, 0, this._private__size.w, this._private__size.h, this._private__options.layout.backgroundColor);
    };
...
var TimeAxisWidget = /** @class */ (function () {
    function TimeAxisWidget(chartWidget) {
        ...
        this._private__canvas = addCanvasTo(this._private__dv, new Size(16, 16));
        this._private__canvas.style.position = 'absolute';
        this._private__canvas.style.zIndex = '1';
        this._private__canvas.style.left = '0';
        this._private__canvas.style.top = '0';
        this._private__canvasContext = ensureNotNull(getContext2d(this._private__canvas));
        this._private__topCanvas = addCanvasTo(this._private__dv, new Size(16, 16));
        this._private__topCanvas.style.position = 'absolute';
        this._private__topCanvas.style.zIndex = '2';
        this._private__topCanvas.style.left = '0';
        this._private__topCanvas.style.top = '0';
        this._private__topCanvasContext = ensureNotNull(getContext2d(this._private__topCanvas));
    ...
    TimeAxisWidget.prototype.setSizes = function (timeAxisSize, stubWidth) {
        if (!this._private__size || !this._private__size.equals(timeAxisSize)) {
            ...
            resizeCanvas(this._private__canvas, timeAxisSize);
            resizeCanvas(this._private__topCanvas, timeAxisSize);
            getContext2d(this._private__canvas); // Sync HiDpiCanvas metrics
            getContext2d(this._private__topCanvas); // Sync HiDpiCanvas metrics
    ...
    TimeAxisWidget.prototype.paint = function (type) {
        ...
        if (type !== 1 /* Cursor */) {
            var ctx = this._private__canvasContext;
            this._private__drawBackground(ctx);
            this._private__drawBorder(ctx);
            this._private__drawTickMarks(ctx);
            this._private__drawBackLabels(ctx);
            if (this._private__stub !== null) {
                this._private__stub.paint(type);
            }
        }
        this._private__drawCrosshairLabel(this._private__topCanvasContext);
    };
    TimeAxisWidget.prototype._private__drawBackground = function (ctx) {
        clearRect(ctx, 0, 0, this._private__size.w, this._private__size.h, this._private__backgroundColor());
    };
    TimeAxisWidget.prototype._private__drawBorder = function (ctx) {
        if (this._private__chart.options().timeScale.borderVisible) {
            ctx.save();
            ctx.fillStyle = this._private__lineColor();
            ctx.translate(-0.5, -0.5);
            ctx.fillRect(0, 0, this._private__size.w, 1);
            ctx.restore();
        }
    };
    TimeAxisWidget.prototype._private__drawTickMarks = function (ctx) {
        var tickMarks = this._private__chart.model().timeScale().marks();
    ...
    TimeAxisWidget.prototype._private__drawBackLabels = function (ctx) {
        ctx.save();
    ...
    TimeAxisWidget.prototype._private__drawCrosshairLabel = function (ctx) {
        this._private__topCanvasContext.clearRect(-0.5, -0.5, this._private__size.w, this._private__size.h);
        var model = this._private__chart.model();
        var views = []; // array of arrays
...
var ChartWidget = /** @class */ (function () {
    ...
    ChartWidget.prototype.takeScreenshot = function () {
        ...
        var targetWidth = this._private__width;
        var targetHeight = this._private__height;
        ...
        if (this._private__options.priceScale.position === 'left') {
            drawPriceAxises();
        ...
        for (var paneIndex = 0; paneIndex < this._private__paneWidgets.length; paneIndex++) {
            ...
            if (paneIndex < this._private__paneWidgets.length - 1) {
                var separator = this._private__paneSeparators[paneIndex];
        ...
        if (this._private__options.priceScale.position === 'right') {
            targetY = 0;
            drawPriceAxises();
        }
        var drawStub = function () {
            var image = ensureNotNull(_this._private__timeAxisWidget.stub()).getImage();
            ctx.drawImage(image, targetX, targetY);
        };
        // draw time scale
        if (this._private__options.timeScale.visible) {
            ...
            var image = this._private__timeAxisWidget.getImage();
            ctx.drawImage(image, targetX, targetY);
            if (this._private__options.priceScale.position === 'right') {
                targetX = firstPane.getSize().w;
                drawStub();
    ...
    ChartWidget.prototype._private__getMouseEventParamsImpl = function (time, point) {
        ...
        var hoveredSeries = hoveredSource !== null && hoveredSource instanceof Series
            ? hoveredSource
            : undefined;
...
var SeriesApi = /** @class */ (function () {
    ...
    SeriesApi.prototype.setMarkers = function (data) {
        var convertedMarkers = data.map(function (marker) { return (__assign({}, marker, { time: convertTime(marker.time) })); });
...
var ChartApi = /** @class */ (function () {
    function ChartApi(container, options) {
        ...
        this._private__chartWidget = new ChartWidget(container, options);
    ...
    ChartApi.prototype.applyOptions = function (options) {
        this._private__chartWidget.applyOptions(options);
    };
